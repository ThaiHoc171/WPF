...
namespace WPF.Pages;
public partial class KhachPage : Page, INotifyPropertyChanged
{
	public KhachPage()
	{		
		InitializeComponent();
	...
	private void SetupColumns()
	{
		...
		GridContent.Columns.Add(new DataGridTextColumn
		{
			Header = "Mã tài khoản",
			Visibility = Visibility.Collapsed,
			Binding = new Binding("TaiKhoanID"),
			Width = new DataGridLength(1, DataGridLengthUnitType.Star)
		});
		GridContent.Columns.Add(new DataGridTextColumn
		{
		...
		GridContent.Columns.Add(SetupDataGrid.CreateIconButtonColumn("AccountPlus", Update_Click, "Tạo bệnh nhân"));
	}
	...
	private async Task LoadData()
	{
		try
		{
			...
			var res = string.IsNullOrWhiteSpace(Keyword)
				? await _client.GetKhachList(Page, SizePage)
	...
	{
		if (sender is Button btn && btn.Tag is ThongTinReadListModel item)
		{
			...
			var win = new CapNhatKhach(item.ThongTinID)
			{
				Owner = parentWindow
			};
	...
	{
		if (sender is not Button btn || btn.Tag is not ThongTinReadListModel item)
			return;

		var confirm = await MessageHelper.Confirm($"Tạo Bệnh Nhân từ khách hàng: {item.HoTen}?");
		if (!confirm) return;

		// Lấy chi tiết khách
		var api = await _client.Detail(item.ThongTinID);
		if (!api.Success || api.Data == null)
		{
			SnackbarHelper.ShowError("Không lấy được thông tin khách hàng!");
			return;
		}
		...
		// Update loại thành bệnh nhân
		var result = await _benhNhanClient.Create(req);
