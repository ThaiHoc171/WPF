...
public partial class NhanVienPage : Page, INotifyPropertyChanged
{
    public NhanVienPage()
    {
        InitializeComponent();
	...
	{
		if (sender is Button btn && btn.Tag is NhanVienReadListModel item)
		{
			var parentWindow = Window.GetWindow(this);
			var overlay = parentWindow.FindName("Overlay") as Border;
			...
			var win = new CapNhatNhanVien(item.NhanVienID)
			{
				Owner = parentWindow
			};
			var result = win.ShowDialog();
