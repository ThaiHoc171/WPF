...
public partial class ThietBiPage : Page, INotifyPropertyChanged
	{
    public ThietBiPage()
    {
        InitializeComponent();
	...
	private async void Import_Click(object sender, RoutedEventArgs e)
	{
		...
		{
			await LoadData();
			SnackbarHelper.ShowSuccess("Thêm thiết bị từ excel thành công!");
		}
	...
	{
		if (sender is Button btn && btn.Tag is ThietBiReadListModel item)
		{
			var parentWindow = Window.GetWindow(this);
			var overlay = parentWindow.FindName("Overlay") as Border;
			...
			var win = new CapNhatThietBi(item.ThietBiID)
			{
				Owner = parentWindow
			};
			var result = win.ShowDialog();
